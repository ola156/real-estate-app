...
export default function ForRentPage() {
  ...
  useEffect(() => {
    const results = listings.filter((item) => {
      ...
      // 2. Property Type Filter (Matches your database column propertyType)
      const matchesType = roomType === "All" || item.propertyType === roomType;

      // 3. Range-Based Price Filtering
      ...
      if (maxPrice !== "Any") {
        ...
        if (!max) {
          matchesPrice = itemPrice >= min; // For "Above 1M"
        } else {
        ...
        {!loading && filteredListings.length === 0 && (
          <div className="text-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-100">
            <p className="text-slate-500 font-medium italic">
              No matches for this specific search. Check back later we have more listings coming in every day!
            </p>
            <button
              onClick={() => {
                setSearchTerm("");
